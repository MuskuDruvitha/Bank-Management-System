...
public class Pin extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        try {        
            String a = new String(t1.getPassword()); // Get the current PIN
            String b = new String(t2.getPassword()); // Get the new PIN
            String c = new String(t3.getPassword()); // Get the re-entered new PIN
            
            // Validate that the PINs are 4 digits long and numeric
            if (ae.getSource() == b1) {
                ...
                if (a.length() != 4 || !a.matches("\\d+")) {
                    JOptionPane.showMessageDialog(null, "Current PIN must be a 4-digit number.");
                    return;
                }
                
                if (b.length() != 4 || !b.matches("\\d+")) {
                    JOptionPane.showMessageDialog(null, "New PIN must be a 4-digit number.");
                    return;
                }
                
                if (c.length() != 4 || !c.matches("\\d+")) {
                    JOptionPane.showMessageDialog(null, "Re-entered PIN must be a 4-digit number.");
                    return;
                }
